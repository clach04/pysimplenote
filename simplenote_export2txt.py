...
def dict2txt(notes_dict, output_directory='notes_export_dir', use_first_line_as_filename=False):
    assert use_first_line_as_filename == False  # dupe detection and resolution needed if True
...
def main(argv=None):
    ...
    if filename.lower().endswith('.json'):
        ...
        f.close()
        notes_dict = json.loads(json_bytes)
    dict2txt(notes_dict, output_directory=filename+'_dir')
